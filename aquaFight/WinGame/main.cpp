...
struct Missile{
	...
	int MissilePositionX2;
	int MissilePositionY2;
...
void CALLBACK TimerProc(HWND hWnd, UINT uMsg, UINT idEvent, DWORD dwTime)
{
	...
	//왼쪽 오른쪽과 충돌체크
	if (player1PositionY + 100 >= Obstacle[0].y && player1PositionY <= Obstacle[0].y + 500 && (player1PositionX <= Obstacle[0].x + 100 || player1PositionX <= Obstacle[0].x + 100))
	{
